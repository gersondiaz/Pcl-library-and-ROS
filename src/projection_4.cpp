...
class Node
{
 public:
...

  {
    ...
    pcl_pub = nh_.advertise<sensor_msgs::PointCloud2>("Proyeccion_5", 1);
  ...
  void callback(const sensor_msgs::PointCloud2ConstPtr& input, const pcl_msgs::ModelCoefficientsConstPtr& input2)
  {
    ...
    coefficients->values[3] = model_coef.values[3] + 0.0285;   //eje z para poder hacer la  proyección
...
{
  ros::init(argc, argv, "Proyeccion_5");
