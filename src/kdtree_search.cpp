...
class Node
{
 public:
  ...
  //:viewer("Cloud Viewer")
  {
    ...
    pcl_pub = nh_.advertise<sensor_msgs::PointCloud2>("Seccion_Danada", 1);
  ...
  void callback(const sensor_msgs::PointCloud2ConstPtr& input, const sensor_msgs::PointCloud2ConstPtr& input2)
  {
   ...
   //ROS_INFO("Synchronization successful");
    ...
    tree.setInputCloud(cloud_B.makeShared());
  ...
  for (std::size_t i = 0; i < cloud_A.points.size(); ++i)
     {
       tree.nearestKSearch (cloud_A.points[i], 1, pointIdxNKNSearch, pointNKNSquaredDistance);
       //std::cout << "    "  <<   (cloud_B)[ pointIdxNKNSearch[i] ]
       //<< " (squared distance: " << pointNKNSquaredDistance[i] << ")" << std::endl;
       if(pointNKNSquaredDistance[0] > 0.000025)
	{
         Diferencia.push_back(cloud_A.points[i]);
...
{
  ros::init(argc, argv, "compara_nubes");
