...
class Node
{
 public:
  ...
  void callback(const sensor_msgs::PointCloud2ConstPtr& input, const sensor_msgs::PointCloud2ConstPtr& input2)
  {
	...
	if (cloud_A.points.size() == 0) {    //init
    	cloud_A = cloud_B;
	return;
	}
	else {
    	if (cloud_B.points.size() > cloud_A.points.size()) {  
        for (size_t i = 0; i < (cloud_A.points.size()); i++) {    //assuming newer points have larger indices
            cloud_B.points[i].x = bad_point;
            cloud_B.points[i].y = bad_point;
            cloud_B.points[i].z = bad_point;    //remove old points
        }

        boost::shared_ptr<std::vector<int>> indices(new std::vector<int>);
        pcl::removeNaNFromPointCloud(cloud_B, *indices);
        
        pcl::ExtractIndices<pcl::PointXYZ> removePrevPts;
        removePrevPts.setInputCloud(cloud_A.makeShared());
        removePrevPts.setIndices(indices);
        removePrevPts.setNegative(false);
        removePrevPts.filter(Diferencia);
        cloud_B += Diferencia;   //reconstruct map
